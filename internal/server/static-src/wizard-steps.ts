...
import {
  type WizardStep, schemaByKey, wizardValues, mediaRoots, infoIcon, wizField, wizToggle,
} from './wizard.js';
...
export function buildArrStep(): WizardStep {
  return {
    ...
    render(container: HTMLElement): void {
      ...
      if (sonarr) renderArrGroup(container, 'sonarr', 'Sonarr', sonarr);
      if (radarr) renderArrGroup(container, 'radarr', 'Radarr', radarr);
    },
    collect(): void { collectArrValues('sonarr'); collectArrValues('radarr'); },
    validate(): string {
      ...
      if (!sf && !rf) return 'At least one of Sonarr or Radarr must be configured';
      if (sf) {
        if (!(sv['url'] || '').trim()) return 'Sonarr URL is required';
        if (!(sv['api_key'] || '').trim()) return 'Sonarr API Key is required';
      }
      if (rf) {
        if (!(rv['url'] || '').trim()) return 'Radarr URL is required';
        if (!(rv['api_key'] || '').trim()) return 'Radarr API Key is required';
      }
      return '';
    },
  };
...
function renderArrGroup(container: HTMLElement, key: string, name: string, section: ConfigSchema): void {
  const header = el('div', { className: 'wiz-arr-header' },
    el('span', null, name));
  const group = el('div', { className: 'wiz-arr-group' }, header);
  const saved = wizardValues[key] || {};
  for (const field of section['fields'] || []) {
    group.appendChild(wizField(
      'wiz-' + key + '-' + field['key'], field['label'],
      field['type'] === 'secret' ? 'secret' : 'text',
      saved[field['key']] ?? '', field['placeholder'] || '', field['help']
    ));
  }
...
function collectArrValues(key: string): void {
  ...
  for (const field of section['fields']!) {
    ...
    if (inp) values[field['key']] = inp.value;
  }
...
export function buildMediaRootsStep(): WizardStep {
  return {
    id: 'media_roots', title: 'Media Roots',
    render(container: HTMLElement): void {
      ...
      if (mediaRoots.length === 0) mediaRoots.push('');
      renderMediaRoots(listContainer);

      container.appendChild(el('button', {
        type: 'button', className: 'wiz-lang-add',
        onclick: () => {
          collectMediaRoots();
          mediaRoots.push('');
          renderMediaRoots(listContainer);
    ...
    collect(): void { collectMediaRoots(); },
    validate(): string {
      collectMediaRoots();
      const valid = mediaRoots.filter((p: string) => p.trim() !== '');
      if (valid.length === 0) return 'At least one media root path is required';
      for (const p of valid) {
        if (!p.startsWith('/')) return 'Media root paths must be absolute (start with /): ' + p;
      }
      return '';
    },
    async validateAsync(signal: AbortSignal): Promise<string> {
      ...
      for (const p of valid) {
        ...
        const data = await apiPost<{ valid: boolean; error?: string }>(
          '/api/config/validate-path', { path: p.trim() }, signal);
        if (signal.aborted) return '';
        if (!data) {
...
function renderMediaRoots(container: HTMLElement): void {
  ...
  for (let i = 0; i < mediaRoots.length; i++) {
    ...
    row.appendChild(el('input', {
      type: 'text', id: 'wiz-media-root-' + String(i),
      className: 'wiz-media-input', placeholder: '/media',
      value: mediaRoots[i] ?? '', autocomplete: 'off',
    ...
    if (mediaRoots.length > 1) {
      const idx = i;
      const rm = el('button', {
        type: 'button', className: 'wiz-lang-remove',
        onclick: () => {
          collectMediaRoots();
...
export function buildSearchStep(): WizardStep {
  return {
    id: 'search', title: 'Search Settings',
    render(container: HTMLElement): void {
      ...
      if (search?.['fields']) {
        const allowed = ['scan_interval', 'exclude_arr_tags', 'upgrade_enabled', 'upgrade_window_days'];
        for (const f of search['fields']!) {
          if (!allowed.includes(f['key'])) continue;
          const sv = savedSearch[f['key']] || f['default'] || '';
          if (f['type'] === 'bool') {
            const checked = sv === 'true';
            container.appendChild(wizToggle(
              'wiz-search-' + f['key'], f['label'], checked, f['help']
            ));
            if (f['key'] === 'upgrade_enabled') {
              ...
              if (windowField) {
                ...
                const windowEl = wizField(
                  'wiz-search-upgrade_window_days', windowField['label'],
                  windowField['type'] || 'number', wv,
                  windowField['placeholder'] || windowField['default'] || '',
                  windowField['help']
                );
                if (!checked) windowEl.hidden = true;
                windowEl.id = 'wiz-search-upgrade-window-row';
                container.appendChild(windowEl);
                const cb = $('wiz-search-upgrade_enabled') as HTMLInputElement | null;
                if (cb) cb.addEventListener('change', () => { windowEl.hidden = !cb.checked; });
              }
            }
          } else if (f['key'] === 'upgrade_window_days') {
            continue;
          } else {
            container.appendChild(wizField(
              'wiz-search-' + f['key'], f['label'], f['type'] || 'text',
              sv, f['placeholder'] || f['default'] || '', f['help']
            ));
          }
        }
      }
      ...
      if (adaptive?.['fields']) {
        ...
        const cb = el('input', { type: 'checkbox', id: 'wiz-adaptive-enabled' }) as HTMLInputElement;
        ...
        if (!enabledChecked) (fieldsDiv as HTMLElement).hidden = true;

        for (const f of adaptive['fields']!) {
          if (f['type'] === 'bool') continue;
          const sv = savedAdaptive[f['key']] || f['default'] || '';
          fieldsDiv.appendChild(wizField(
            'wiz-adaptive-' + f['key'], f['label'], f['type'] || 'text',
            sv, f['placeholder'] || f['default'] || '', f['help']
          ));
        }
        container.appendChild(fieldsDiv);

        cb.addEventListener('change', () => { fieldsDiv.hidden = !cb.checked; });
      }
    },
    collect(): void {
      const search = schemaByKey('search');
      if (search?.['fields']) {
        ...
        for (const f of search['fields']!) {
          ...
          if (!inp) continue;
          if (f['type'] === 'bool') vals[f['key']] = String((inp as HTMLInputElement).checked);
          else vals[f['key']] = (inp as HTMLInputElement).value;
        }
      ...
      if (adaptive?.['fields']) {
        ...
        if (cb) vals['enabled'] = String(cb.checked);
        for (const f of adaptive['fields']!) {
          if (f['type'] === 'bool') continue;
          const inp = $('wiz-adaptive-' + f['key']) as HTMLInputElement | null;
          if (inp) vals[f['key']] = inp.value;
        }
...
export function buildPostProcessStep(): WizardStep {
  return {
    id: 'post_processing', title: 'Post-Processing',
    render(container: HTMLElement): void {
      ...
      for (const field of section['fields']!) {
        ...
        if (field['type'] === 'bool') {
          ...
          container.appendChild(wizField(
            'wiz-pp-' + field['key'], field['label'], field['type'] || 'text',
            savedVal ?? '', field['placeholder'] || field['default'] || '', field['help']
          ));
        }
      }
      ...
      if (syncCb && audioCb) {
        const updateAudio = (): void => {
          ...
          if (!syncCb.checked) audioCb.checked = false;
        };
        updateAudio();
    ...
    collect(): void {
      ...
      for (const field of section['fields']!) {
        ...
        if (!inp) continue;
        if (field['type'] === 'bool') values[field['key']] = String((inp as HTMLInputElement).checked);
        else values[field['key']] = (inp as HTMLInputElement).value;
      }
