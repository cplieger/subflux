...
function buildMenuContent(): void {
  ...
  if (userInfo) {
    items.push(el('div', { className: 'um-user', role: 'none' },
      el('span', { className: 'um-name' }, userInfo.username)));
  }

  // Security link.
  items.push(menuItem('Security', 'settings', () => {
    popup.hidePopover();
  ...
  if (userInfo && userInfo.role === 'admin') {
    items.push(menuItem('Settings', 'settings', () => {
      popup.hidePopover();
      openConfig();
    }));
  }
