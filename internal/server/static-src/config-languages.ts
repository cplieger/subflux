...
function cfgFieldEl(
  ...
  if (tip) lbl.setAttribute('data-tip', tip);
  return el('div', { className: 'cfg-field' },
    lbl, element);
}
...
export function buildLanguagesSection(): HTMLElement {
  ...
  // --- Defaults ---
  sec.appendChild(el('div', { className: 'cfg-title' },
    'Language Defaults'));
  const defaultsContainer = el('div', {
    id: 'lang-defaults', className: 'lang-subs'
  });
  ...
  sec.appendChild(el('button', {
    ...
    onclick: () => defaultsContainer.appendChild(
      buildSubTarget({ code: 'en' }, true))
  }, '+ Add subtitle'));
  ...
  sec.appendChild(el('button', {
    type: 'button', className: 'ghost',
    onclick: () => rulesContainer.appendChild(
      buildRuleBlock({
        audio: 'en',
        subtitles: [{ code: 'fr' }]
      }))
...
function buildSubTarget(sub: SubTarget, isDefault: boolean): HTMLElement {
  ...
  if (!isDefault) {
    row.appendChild(variantSelect(null,
      sub.variant || DEFAULT_VARIANT));
  }
  ...
  if (!isDefault) {
    const ms = el('input', {
      type: 'number', className: 'lang-min-score',
      placeholder: 'min score', autocomplete: 'off',
      value: sub.min_score != null ? String(sub.min_score) : ''
    });
    adv.appendChild(cfgFieldEl('Min score', ms,
      'Override global minimum score for this target'));
  }
  const prov = el('input', {
    type: 'text', className: 'lang-providers',
    placeholder: 'providers (comma-sep)', autocomplete: 'off',
    value: (sub.providers || []).join(', ')
  });
  adv.appendChild(cfgFieldEl('Providers', prov,
    'Only use these providers (empty = all)'));
  const excl = el('input', {
    type: 'text', className: 'lang-exclude',
    placeholder: 'exclude (comma-sep)', autocomplete: 'off',
    value: (sub.exclude || []).join(', ')
  });
  adv.appendChild(cfgFieldEl('Exclude', excl,
    'Skip these providers'));

  // Only show advanced if any field has a value.
  const hasAdvanced = (sub.min_score != null)
    || (sub.providers && sub.providers.length > 0)
    || (sub.exclude && sub.exclude.length > 0);
  if (!hasAdvanced) adv.style.display = 'none';
...
function buildRuleBlock(rule: LanguageRule): HTMLElement {
  ...
  block.appendChild(
    el('span', { className: 'lang-label' }, 'Audio:'));
  ...
  block.appendChild(
    el('span', { className: 'lang-label' }, 'Subtitles:'));
  ...
  block.appendChild(el('button', {
    type: 'button', className: 'ghost',
    onclick: () => subsContainer.appendChild(
      buildSubTarget({ code: 'en' }, false))
  }, '+ Add subtitle'));
...
export function serializeLanguagesFromForm(): string {
  ...
  if (rulesEl && rulesEl.children.length > 0) {
    ...
    for (const block of Array.from(rulesEl.children)) {
      const audioSel = block.querySelector('.lang-row .lang-select') as HTMLSelectElement | null;
      if (!audioSel) continue;
...
function serializeSubTarget(
  ...
  const langSel = row.querySelector('.lang-select') as HTMLSelectElement | null;
  if (!langSel) return;
  const indentStr = ' '.repeat(indent);
  lines.push(`${indentStr}- code: ${langSel.value}`);
  if (!isDefault) {
    const varSel = row.querySelector('.variant-select') as HTMLSelectElement | null;
    const v = varSel ? varSel.value : DEFAULT_VARIANT;
    if (v !== DEFAULT_VARIANT) {
      lines.push(`${indentStr}  variant: ${v}`);
    }
    const msEl = row.querySelector('.lang-min-score') as HTMLInputElement | null;
    if (msEl && msEl.value) {
      lines.push(`${indentStr}  min_score: ${msEl.value}`);
    }
  }
  const provEl = row.querySelector('.lang-providers') as HTMLInputElement | null;
  if (provEl && provEl.value.trim()) {
    const provs = provEl.value.split(',')
      .map((s: string) => s.trim()).filter(Boolean);
    lines.push(`${indentStr}  providers: [${provs.join(', ')}]`);
  }
  const exclEl = row.querySelector('.lang-exclude') as HTMLInputElement | null;
  if (exclEl && exclEl.value.trim()) {
    const excls = exclEl.value.split(',')
      .map((s: string) => s.trim()).filter(Boolean);
