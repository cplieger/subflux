...
export function showTip(target: HTMLElement): void {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (i > 0) tipEl.appendChild(el('br'));
    const line = lines[i];
    if (line !== undefined) tipEl.appendChild(text(line));
  }
...
export function hideTip(): void {
  ...
  tipTimer = null;
  if (tipEl) {
    const anchored = document.querySelector('[style*="--tip-anchor"]') as HTMLElement | null;
...
document.addEventListener('pointerover', (e: PointerEvent) => {
  ...
  const target = t.closest('[data-tip]') as HTMLElement | null;
