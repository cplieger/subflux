...
import {
  type WizardStep, schemaByKey, wizardValues, providerEnabled, wizField, wizToggle,
} from './wizard.js';
...
export function buildProvidersStep(): WizardStep {
  return {
    ...
    render(container: HTMLElement): void {
      ...
      if (embedded) renderProviderCard(container, embedded, true);
      for (const prov of section['providers']) {
        if (prov['always_enabled']) continue;
        renderProviderCard(container, prov, false);
      }
    },
    collect(): void { collectProviders(); },
    validate(): string {
      collectProviders();
      const section = schemaByKey('providers');
      if (!section?.['providers']) return '';
      for (const prov of section['providers']) {
        if (!providerEnabled[prov['name']]) continue;
        if (prov['always_enabled']) continue;
        ...
        if (!hasAnyCred) {
          return prov['label'] + ' is enabled but has no credentials configured. '
...
function renderProviderCard(container: HTMLElement, prov: ProviderSchema, isAlways: boolean): void {
  ...
  if (!isAlways) {
    const cb = el('input', { type: 'checkbox', id: 'wiz-prov-' + prov['name'] }) as HTMLInputElement;
    cb.checked = enabled;
    const toggle = el('label', { className: 'wiz-toggle' }, cb, el('span'));
    header.appendChild(toggle);
    cb.addEventListener('change', () => {
      card.classList.toggle('open', cb.checked);
      const body = card.querySelector('.wiz-prov-body') as HTMLElement | null;
  ...
  if (settings && settings.length > 0) {
    ...
    if (!enabled) (body as HTMLElement).hidden = true;
    ...
    for (const f of settings) {
      ...
      if (f['type'] === 'bool') {
        ...
        body.appendChild(wizToggle(
          'wiz-prov-' + prov['name'] + '-' + f['key'],
          f['label'], checked, f['help']
        ));
      } else {
        body.appendChild(wizField(
          'wiz-prov-' + prov['name'] + '-' + f['key'], f['label'],
          f['secret'] ? 'secret' : (f['type'] || 'text'),
          savedVal, f['placeholder'] || f['default'] || '', f['help']
        ));
      }
    }
...
function collectProviders(): void {
  ...
  if (!section?.['providers']) return;
  for (const prov of section['providers']) {
    if (prov['always_enabled']) { providerEnabled[prov['name']] = true; }
    else {
      const cb = $('wiz-prov-' + prov['name']) as HTMLInputElement | null;
      if (cb) providerEnabled[prov['name']] = cb.checked;
    }
    const settings = prov['settings'];
    if (settings && settings.length > 0) {
