...
function mockFetch(response: Partial<Response>) {
  const r = {
    ...
    headers: new Headers(response.headers as HeadersInit ?? { 'Content-Type': 'application/json' }),
...
describe('apiDelete', () => {
  it('returns true on success', async () => {
    expect.assertions(1);
    mockFetch({ ok: true, status: 200, headers: { 'Content-Type': 'application/json' } as unknown as Headers, json: () => Promise.resolve(null) });
  ...
  it('returns false on failure', async () => {
    expect.assertions(1);
    mockFetch({ ok: false, status: 403, statusText: 'Forbidden', headers: { 'Content-Type': 'application/json' } as unknown as Headers, json: () => Promise.resolve({ error: 'forbidden' }) });
    const spy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const r = await apiDelete('/api/item/1');
    expect(r).toBe(false);
    spy.mockRestore();
...
describe('typed error envelope', () => {
  it('exposes code and requestId on 4xx with full envelope', async () => {
    ...
    mockFetch({
      ...
      headers: { 'Content-Type': 'application/json' } as unknown as Headers,
      json: () => Promise.resolve({ error: 'invalid input', code: 'config_invalid', request_id: 'req-abc-123' }),
    });
