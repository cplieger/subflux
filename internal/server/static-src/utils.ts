...
const use12h = (() => {
  try {
    const sample = new Intl.DateTimeFormat(undefined, { hour: 'numeric' })
      .resolvedOptions();
...
export function fmtTime(d: Date): string { return timeFmt.format(d); }
...
export function viewTransition(fn: () => void): void {
  ...
  const run = (): void => {
    ...
    pending = t.finished.then(() => { pending = null; });
    t.ready.catch(() => {});
    t.finished.catch(() => { pending = null; });
  };
...
export function clickableRow(handler: () => void, ...children: Array<string | Node | null | undefined>): HTMLElement {
  return el('tr', {
    className: 'clickable',
    tabindex: '0',
    role: 'link',
...
export function emptyState(msg: string, actionLabel?: string, actionFn?: () => void): HTMLElement {
  const frag = el('div', { className: 'empty' },
    el('div', null, msg));
...
export function coverageMediaId(item: { _type: 'series' | 'movie'; tvdb_id?: number; tmdb_id?: number }): string {
  return item._type === 'series'
    ? `tvdb-${item.tvdb_id}` : `tmdb-${item.tmdb_id}`;
}
...
export function langName(code: string): string {
  if (!code || code === 'default' || code === 'no targets') return code;
