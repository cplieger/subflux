...
export function renderProvidersSection(
  ...
  for (const prov of schema.providers) {
    if (prov.name === EMBEDDED_PROVIDER) continue;
    const block = blocks[prov.name] || '';
    const card = el('div', { className: 'provider' });
    const isEnabled = block !== ''
      && extractYAMLValue(block, 'enabled') !== 'false';

    const headerItems: HTMLElement[] = [
      el('span', null, prov.label)
    ];
    headerItems.push(
      cfgToggle(`cfg-prov-${prov.name}-enabled`, isEnabled));
    card.appendChild(el('div', { className: 'provider-head' },
      ...headerItems));

    const details = el('div', { className: 'provider-body' });
    if (!isEnabled) details.style.display = 'none';

    const priVal = extractYAMLValue(block, 'priority');
    details.appendChild(cfgField(
      `cfg-prov-${prov.name}-priority`, 'Priority', 'number',
      priVal || '', '99',
    ...
    const toggle = card.querySelector(`#cfg-prov-${prov.name}-enabled`) as HTMLInputElement | null;
    if (toggle) {
  ...
  apiGet<ProviderHealthResponse>('/api/providers/timeout')
    .then((data: ProviderHealthResponse | null) => {
      ...
      for (const [name, status] of Object.entries(data.providers)) {
        if (name === EMBEDDED_PROVIDER) continue;
        const header = sec.querySelector(
          `#${CSS.escape(`cfg-prov-${name}-enabled`)}`);
        if (!header) continue;
...
export function renderEmbeddedSection(
  ...
  for (const sf of embProv.settings) {
    ...
    let val = '';
    if (settingsRaw) val = extractYAMLValue(settingsRaw, sf.key);
  ...
  for (const prov of schema.providers) {
    ...
    if (!prov.always_enabled) {
      const enEl = document.getElementById(
        `cfg-prov-${prov.name}-enabled`) as HTMLInputElement | null;
      lines.push(`    enabled: ${enEl ? String(enEl.checked) : 'false'}`);
      const priEl = document.getElementById(
        `cfg-prov-${prov.name}-priority`) as HTMLInputElement | null;
    ...
    if (prov.settings && prov.settings.length > 0) {
      ...
      for (const sf of prov.settings) {
        const fEl = document.getElementById(
          `cfg-prov-${prov.name}-s-${sf.key}`) as HTMLInputElement | null;
        if (!fEl) continue;
        let val: string;
        if (fEl.type === 'checkbox') val = String(fEl.checked);
        else if (sf.secret && !fEl.value) val = '""';
        else val = fEl.value || '""';
