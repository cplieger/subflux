...
import {
  type WizardStep, langRules, langDefault, infoIcon, langSelect, variantSelect,
} from './wizard.js';
...
export function buildLanguagesStep(): WizardStep {
  return {
    ...
    render(container: HTMLElement): void {
      container.appendChild(el('div', { className: 'wiz-lang-section-title' },
      ...
      if (langDefault.length === 0) langDefault.push({ code: '', variant: DEFAULT_VARIANT });
    ...
    collect(): void { collectLangDefaults(); collectLangRules(); },
    validate(): string {
      collectLangDefaults(); collectLangRules();
...
function renderLangDefaults(container: HTMLElement): void {
  ...
  for (let i = 0; i < langDefault.length; i++) {
    ...
    row.appendChild(langSelect('wiz-lang-def-code-' + String(i), langDefault[i]!.code, 'Subtitle language'));
...
function renderLangRules(container: HTMLElement): void {
  ...
  for (let i = 0; i < langRules.length; i++) {
    const row = el('div', { className: 'wiz-lang-row' });
    row.appendChild(langSelect('wiz-lang-rule-audio-' + String(i), langRules[i]!.audio, 'Audio language'));
    row.appendChild(el('span', { className: 'wiz-lang-arrow' }, '\u2192'));
    row.appendChild(langSelect('wiz-lang-rule-code-' + String(i), langRules[i]!.code, 'Subtitle language'));
...
function collectLangDefaults(): void {
  for (let i = 0; i < langDefault.length; i++) {
    ...
    if (c) langDefault[i]!.code = c.value;
    if (v) langDefault[i]!.variant = v.value;
  }
}
...
function collectLangRules(): void {
  for (let i = 0; i < langRules.length; i++) {
    ...
    if (a) langRules[i]!.audio = a.value;
    if (c) langRules[i]!.code = c.value;
    if (v) langRules[i]!.variant = v.value;
  }
}
