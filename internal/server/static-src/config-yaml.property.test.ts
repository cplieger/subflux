...
describe('config-yaml property tests', () => {
  it('parseYAMLSections: output keys are exactly the section headers', () => {
    fc.assert(
      fc.property(
        fc.array(fc.constantFrom('server', 'providers', 'languages', 'adaptive', 'sync'), { minLength: 1, maxLength: 5 }),
  ...
  it('extractYAMLValue: round-trips simple scalars', () => {
    fc.assert(
      fc.property(
        ...
        fc.string({ minLength: 1, maxLength: 30 }).filter(v => !v.includes('#') && !v.includes('\n') && !v.includes(':') && !v.includes('"') && !v.includes("'") && !v.startsWith(' ') && !v.endsWith(' ')),
        (key, value) => {
