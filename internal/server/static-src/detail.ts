...
function coverageBadge(entries: SubtitleEntry[] | null, langLabel: string | null): HTMLElement {
  if (!entries || entries.length === 0) {
    if (langLabel) {
      return el('span', { className: 'badge badge-split', 'data-status': 'err' },
  ...
  for (const sub of entries) {
    ...
    if (!byCodec[codec]) byCodec[codec] = [];
    byCodec[codec]!.push(sub.source === EMBEDDED_PROVIDER ? 'emb' : 'ext');
  }

  const allIgnored = entries.every(sub =>
    sub.source === EMBEDDED_PROVIDER
    && store.get('ignoredCodecs').has(sub.codec || ''));
  const badgeStatus = allIgnored ? 'warn' : 'ok';

  const parts = Object.entries(byCodec).map(([codec, sources]) =>
    `${codec}: ${sources.join(' \u00B7 ')}`);
  let detail = parts.join(' + ');

  const topScore = Math.max(...entries.map(s => s.score || 0));
  if (topScore > 0) detail += ` \u2605${topScore}`;
...
function openSeriesDetail(s: SeriesItem, skipPush?: boolean): void {
  ...
  if (!skipPush) history.pushState(null, '', `/series/${  s.tvdb_id}`);
  const targets = s.targets || [];
  const subsInfo = targets.length > 0
    ? targets.map(t => fmtLangVariant(t.language, t.variant)).join(', ')
    : '';
  const info = `${s.episodes  } ep \u00B7 audio: ${
     langName(s.rule || 'default')
      ...
      if (entries) {
        for (const sub of entries) {
          if (sub.source !== EMBEDDED_PROVIDER && sub.path) {
            result.push({
              subPath: sub.path, videoPath: ep.path,
...
function buildEpisodeRow(
  series: SeriesItem, sg: SeasonGroup, ep: Episode,
  subIdx: Record<string, Partial<Record<string, SubtitleEntry[]>>>,
  targetLangs: Array<{lang: string; variant: string}>,
  hasAbsOrder: boolean, historySet: Set<string>,
): HTMLElement {
  const mediaId = tvdbMediaId(series.tvdb_id, sg.season, ep.episode);
  const subs = subIdx[mediaId] || {};

  const langCells = targetLangs.length > 0
    ? targetLangs.map(t => {
        const key = `${t.lang  }|${  t.variant}`;
        const entries = subs[key];
    ...
    : [el('td', null, Object.keys(subs).length > 0
        ? el('span', { className: 'badge', 'data-status': 'ok' },
  ...
  const epLabel = hasAbsOrder && absEp
    ? `#${  absEp}`
    : `E${  pad(airedEp)}`;

  const epNumCell = hasAbsOrder && absEp
    ? el('td', { className: 'ep-num',
        'data-tip': `Absolute #${  absEp
           }, aired ${fmtEpisode(sg.season, airedEp)}`
      }, epLabel,
        el('span', { className: 'ep-aired' },
          ` E${  pad(airedEp)}`))
  ...
  for (const t of targetLangs) {
    ...
    if (entries) {
      for (const sub of entries) {
        if (sub.source !== EMBEDDED_PROVIDER && sub.path) extEpSubs.push(sub);
      }
    }
  }
  ...
  const syncBtn = extEpSubs.length > 0 && firstExtSub
    ? el('button', {
        type: 'button', className: 'ghost',
        'data-tip': 'Adjust subtitle timing',
        onclick: () => openSyncDialog(
          extEpSubs, ep.path || '', 'series', series.id,
  ...
  return el('tr', null,
    epNumCell,
    el('td', { className: 'ep-title',
      'data-ep': epLabel }, ep.title || ''),
    covCell,
    el('td', { 'data-col': 'actions' },
      el('div', { className: 'action-group' },
        syncBtn, histBtn, searchBtn))
  );
}
...
export function renderSeriesDetail(
  ...
  if (!historySet) historySet = new Set();
  ...
  if (headerEl) {
    ...
    if (hasExtSubs) {
      ...
      const filesBtn = el('button', {
        type: 'button', className: 'ghost',
        'data-nav': 'files',
        onclick: () => openFileManager('episode',
          `tvdb-${series.tvdb_id}-`, series.title,
          `/series/${series.tvdb_id}`, epPaths, series.id)
      }, icon('file'), el('span', { className: 'btn-text' }, ' Files'));
  ...
  const targetLangs = ls.map(t => ({
    lang: t.language, variant: t.variant
  }));
  ...
  for (const [mediaId, files] of Object.entries(byMedia)) {
    subIdx[mediaId] = Object.groupBy(files || [],
      f => `${f.language}|${f.variant}`);
  }
  ...
  const hasAbsOrder = (() => {
    ...
    for (const sg of sortedSeasons) {
      if (sg.season === 0) continue;
      const eps = sg.episodes || [];
      for (const ep of eps) {
        if (ep.absolute_episode
            && ep.absolute_episode !== prior + ep.episode) return true;
      }
      prior += eps.length;
    }
    return false;
...
export function openMovieDetail(m: MovieDetail, skipPush?: boolean): void {
  ...
  if (!skipPush) history.pushState(null, '', `/movie/${  m.tmdb_id}`);
  const targets = m.targets || [];
  const subsInfo = targets.length > 0
    ? targets.map(t => fmtLangVariant(t.language, t.variant)).join(', ')
    : '';
  const info = `${m.year || ''  } \u00B7 audio: ${
     langName(m.rule || 'default')
     }${subsInfo ? ` \u00B7 subs: ${  subsInfo}` : ''}`;
  const subs = m.subs || [];
  ...
  emit(BusEvent.PanelConfigure, false, {
    ...
    filesAction: hasExtSubs
      ? () => openFileManager('movie',
          `tmdb-${m.tmdb_id}`, m.title, `/movie/${m.tmdb_id}`,
          new Map([['', m.path || '']]), m.id)
      : null
  });
      ...
      if (ids && ids.length > 0) {
        ...
        if (headerEl && !headerEl.querySelector('[data-nav="hist"]')) {
          const histBtn = el('button', {
            type: 'button', className: 'ghost',
            'data-nav': 'hist',
            onclick: () => emit(BusEvent.NavHistory, m.title)
          }, icon('history'), el('span', { className: 'btn-text' }, ' History'));
  ...
  if (extSubs.length > 0 && firstExtSub) {
    const syncBtn = el('button', {
      ...
      'data-tip': 'Adjust subtitle timing',
      onclick: () => openSyncDialog(
        extSubs, m.path || '', 'movie', m.id, m.title)
    }, icon('sync'), el('span', { className: 'btn-text' }, ' Sync'));
    insertNavButton(syncBtn);
  }

  if (targets.length === 0) {
    frag.appendChild(el('span', { className: 'badge', 'data-status': 'muted' },
      'not scanned'));
  } else {
    const subIdx = Object.groupBy(subs,
      s => `${s.language}|${s.variant}`);

    const tbody = el('tbody');
    for (const t of targets) {
      const key = `${t.language  }|${  t.variant}`;
      const entries = (subIdx as Record<string, SubtitleEntry[] | undefined>)[key] || [];
      const label = langName(t.language) + (t.variant !== DEFAULT_VARIANT
        ? ` (${  t.variant  })` : '');
    ...
    frag.appendChild(
      el('table', { className: 'movie-detail' },
...
on(BusEvent.ScanSeries, (item, btn) =>
  triggerSeriesScan(item as SeriesItem, btn));
on(BusEvent.ScanMovie, (item, btn) =>
  triggerMovieScan(item as MovieDetail, btn));
