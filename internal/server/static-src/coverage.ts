...
export async function loadCoverage(silent?: boolean): Promise<void> {
  if (store.get('isUnconfigured')) return;
  ...
  try {
    ...
    if (!silent) patch(out, errDiv(msg));
  }
}
...
export function configurePanel(visible: boolean, detail?: DetailConfig): void {
  ...
  const headerEl = $.coveragePanel.querySelector('.card-head') as HTMLElement | null;
  if (!headerEl) return;
  headerEl.hidden = false;

  // Remove any previous detail nav elements.
  headerEl.querySelectorAll('[data-nav]').forEach(
    (e: Element) => e.remove());
  ...
  if (heading) {
    ...
    if (detail.info) {
      heading.appendChild(el('span', { className: 'detail-info' },
        detail.info));
    }
  }
  ...
  }, icon('arrow-left'), el('span', { className: 'btn-text' }, ' Back'));
  ...
  if (detail.arrLink) {
    ...
    headerEl.appendChild(el('button', {
      ...
      'data-nav': 'arr',
      onclick: () => { if (detail.arrLink) window.open(detail.arrLink, '_blank', 'noopener'); }
    }, icon('external'), ` ${  name}`));
  }
...
function buildBadges(targets: CoverageTarget[]): DocumentFragment {
  ...
  if (!targets || targets.length === 0) {
    frag.appendChild(
      el('span', { className: 'badge', 'data-status': 'muted' },
        'not scanned'));
    return frag;
  }
  for (const t of targets) {
    ...
    const ignoredPct = t.total > 0
      ? (t.have_ignored || 0) / t.total : 0;
    let status = 'err';
    if (pct >= 1) status = 'ok';
    else if (pct > 0) status = 'partial';
    else if (ignoredPct > 0) status = 'warn';
    const displayHave = t.have + (t.have_ignored || 0);
    frag.appendChild(el('span', {
      className: 'badge badge-split',
      'data-status': status,
      'data-tip': ignoredPct > 0 && pct < 1
        ? `${t.have_ignored  } with ignored codec only`
        : undefined
    },
...
function renderCoverageItems(items: CoverageItem[]): DocumentFragment {
  ...
  if (items.length === 0 && (!covData || covData.length === 0)) {
    ...
    frag.appendChild(emptyState(
      'No media found. Data will appear after the first scheduled scan.'));
    return frag;
  }
