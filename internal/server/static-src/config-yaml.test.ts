...
import {
  parseYAMLSections, extractYAMLValue, extractYAMLBlock,
  parseProviderBlocks, formatDurationCfg
} from './config-yaml.js';
...
describe('extractYAMLValue', () => {
  const cases = [
    { name: 'extracts simple value', raw: 'enabled: true\npriority: 5', key: 'enabled', expected: 'true' },
    { name: 'extracts numeric value', raw: 'priority: 99', key: 'priority', expected: '99' },
    { name: 'strips surrounding quotes', raw: 'api_key: "abc123"', key: 'api_key', expected: 'abc123' },
    { name: 'strips single quotes', raw: "name: 'test'", key: 'name', expected: 'test' },
    { name: 'strips inline comments', raw: 'timeout: 30s # seconds', key: 'timeout', expected: '30s' },
    { name: 'returns empty for missing key', raw: 'foo: bar', key: 'baz', expected: '' },
    { name: 'returns empty for comment-only value', raw: 'key: # nothing', key: 'key', expected: '' },
  ];
