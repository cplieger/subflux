...
const SEARCH_ERROR_MAP: ReadonlyArray<{ code: ErrorCode; msg: string; empty?: boolean }> = [
  { code: ErrorCode.SearchProviderDisabled, msg: 'All search providers are disabled. Enable at least one in settings.' },
  { code: ErrorCode.SearchNoResults, msg: 'No results found from any provider.', empty: true },
  { code: ErrorCode.ProviderTimedOut, msg: 'This provider is in cooldown; try again in a few minutes.' },
];
...
export function openSearchPopup(mediaType: MediaType, media: CoverageMedia,
    season: number | null, episode: CoverageEpisode | null,
    lang?: string | null): void {
  const cfg = store.get('config');
  const langs: string[] = (cfg && cfg.languages) || ['en'];
  const defaultLang: string = lang || langs[0] || 'en';

  // Push search URL.
  const idPart: string = mediaType === 'episode'
    ? `/series/${  media.tvdb_id}` : `/movie/${  media.tmdb_id}`;
  const searchPath: string = `${idPart  }/search/${  defaultLang}`;
  ...
  if (mediaType === 'episode' && episode) {
    title += ` ${fmtEpisode(season as number, episode.episode)}`;
  }

  const langSel = el('select', {
    id: 'popup-lang-sel', 'aria-label': 'Language'
  }) as HTMLSelectElement;
  for (const l of langs) langSel.appendChild(option(l, langName(l)));
  langSel.value = defaultLang;
  langSel.addEventListener('change', () => {
    history.replaceState(null, '',
      `${idPart  }/search/${  langSel.value}`);
    runPopupSearch(mediaType, media, season, episode,
      langSel.value);
  });

  const header: HTMLElement = el('div', { className: 'dlg-head' },
  ...
  const resultsDiv: HTMLElement = el('div', {
    id: 'popup-search-results', className: 'dlg-body'
  });
  ...
  // Auto-run search.
  runPopupSearch(mediaType, media, season, episode, defaultLang);
}
...
async function runPopupSearch(mediaType: MediaType, media: CoverageMedia,
    season: number | null, episode: CoverageEpisode | null,
  ...
  if (mediaType === 'episode') {
    ...
    if (media.imdb_id) params.set('imdb', media.imdb_id);
    ...
    if (episode) {
      ...
      if (episode.absolute_episode)
        params.set('absolute_episode',
          String(episode.absolute_episode));
      if (episode.title)
        params.set('episode_title', episode.title);
      if (episode.scene_name)
        params.set('release', episode.scene_name);
      if (episode.path) params.set('file', episode.path);
    }
    ...
    params.set('title', media.title);
    if (media.year) params.set('year', String(media.year));
    if (media.scene_name) params.set('release', media.scene_name);
    if (media.path) params.set('file', media.path);
  }

  const r = await apiGetRaw<SearchResponse | SearchResult[]>(`/api/search?${params.toString()}`, signal);
  if (!r.ok) {
    if (r.status === 0 && signal.aborted) return;
  ...
  renderPopupResults(out, results, lang, mediaType, media,
    season, episode);
}

function renderPopupResults(out: HTMLElement, results: SearchResult[],
    lang: string, mediaType: MediaType, media: CoverageMedia,
    season: number | null, episode: CoverageEpisode | null): void {
  ...
  frag.appendChild(el('div', { className: 'muted result-count' },
    `${results.length  } result(s)`));

  // Header row.
  frag.appendChild(el('div', { className: 'result-row muted' },
  ...
  for (let i = 0; i < visible.length; i++) {
    ...
    const dlBtn: HTMLButtonElement = el('button', {
      type: 'button',
      ...
      onclick: () => {
        downloadFromPopup(dlBtn, {
          sub: s, lang, mediaType, media, season, episode, isTop
        });
      }
    }, icon('download')) as HTMLButtonElement;
    ...
    const scoreTip: string = keys.length > 0
      ? keys.map((k: string) =>
        `${prettyLabel(k)  }: ${  matches[k]!}`).join('\n')
      : 'No attribute matches';

    const row: HTMLElement = el('div', {
      ...
      el('span', { className: 'result-provider' },
        s.provider, s.on_disk ? el('span', null, ' ', icon('check')) : null),
      el('span', { className: 'result-match' }, s.matched_by || ''),
      el('span', { className: 'result-release' },
        `${s.release_name || ''  } `,
        s.hearing_impaired
          ? el('span', { className: 'result-hi' }, '[HI]') : null),
...
async function downloadFromPopup(btn: HTMLElement, opts: DownloadOpts): Promise<void> {
  ...
  let filePath = '';
  if (mediaType === 'episode' && episode && episode.path) {
    filePath = episode.path;
  } else if (mediaType === 'movie' && media && media.path) {
    filePath = media.path;
  }
  if (!filePath) {
    notify.error('File path not available for download. '
  ...
  async function pollDownload(): Promise<void> {
    ...
    if (!acts) {
      if (!signal.aborted) setTimeout(pollDownload, DOWNLOAD_POLL_MS);
      return;
    }
    const act: Activity | undefined = acts.find((a: Activity) => a.id === actID);
    if (act) seenActivity = true;
    if (act && !act.done) {
      if (!signal.aborted) setTimeout(pollDownload, DOWNLOAD_POLL_MS);
      return;
    }
