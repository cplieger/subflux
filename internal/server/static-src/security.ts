...
async function openSecurity(): Promise<void> {
  const dlg = secDlg();
  const closeFn = (): void => { closeDialog(dlg); };
...
function showReauthPrompt(): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    const header = dialogHead('Reauthenticate', () => settle(false));
    ...
    dlg.showModal();
    onBackdropClose(dlg, () => settle(false));
...
function buildPasswordSection(): HTMLElement {
  ...
  sec.appendChild(el('div', { className: 'sec-fields' },
    el('label', null, 'Current password'), currentPw,
    el('label', null, 'New password'), newPw));
...
function buildTOTPSection(me: MeResponse | null): HTMLElement {
  ...
  const status = el('div', { className: 'sec-status' },
    el('span', null, 'Status: '),
    el('span', {
      className: 'badge',
    ...
    const disableBtn = el('button', {
      ...
      onclick: busyClick(async () => {
        ...
        if (r.ok) {
          ...
          await renderSections(secDlg().querySelector('.dlg-body') as HTMLElement);
        } else {
...
function showTOTPSetup(container: HTMLElement, data: TOTPEnableResponse): void {
  const uriDisplay = el('div', { className: 'sec-totp-uri' },
    ...
    onclick: busyClick(async () => {
      ...
      if (r.ok && r.data) {
        ...
        if (r.data.recovery_codes && r.data.recovery_codes.length > 0) {
          ...
          renderSections(secDlg().querySelector('.dlg-body') as HTMLElement);
        }
      } else {
  ...
  container.replaceChildren(uriDisplay,
    el('div', { className: 'sec-fields' },
      el('label', null, 'Verification code'), codeInput),
    feedback,
...
function prepareCreationOptions(
  ...
  if (pk.user && typeof pk.user.id === 'string') {
    pk.user.id = base64urlToBuffer(pk.user.id as unknown as string);
  }
...
async function registerPasskey(): Promise<void> {
  try {
    ...
    if (extensions['credProps'] && extensions['credProps']['rk'] === false) {
      notify.error('Your authenticator created a non-discoverable credential. Passwordless login may not work with this passkey.');
    }
...
function showNewAPIKey(container: HTMLElement, key: string): void {
  const keyDisplay = el('div', { className: 'sec-new-key' },
    el('p', null, 'Copy this key now. It will not be shown again.'),
    el('div', { className: 'sec-key-value' },
      el('code', null, key),
      el('button', {
        type: 'button', className: 'ghost',
        onclick: () => {
  ...
  if (insertPoint) {
    ...
    container.insertBefore(el('div', { className: 'sec-actions' }, doneBtn), keyDisplay.nextSibling);
  } else {
...
function showInputDialog(
  ...
  return new Promise((resolve) => {
    ...
    const header = dialogHead('Input', () => settle(null));
    ...
    dlg.replaceChildren(header, body, footer);
    dlg.showModal();
    onBackdropClose(dlg, () => settle(null));
    dlg.addEventListener('close', () => settle(null), { once: true });
    inp.focus();
  });
}
