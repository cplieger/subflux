...
export default defineConfig({
  test: {
    ...
    coverage: {
      ...
      // not. The `**/` is explicit and load-bearing: vitest 5 matches
      // include/exclude against the root-relative path without picomatch's
      // `contains`, so the bare `*.ts` this replaced reached only the top level.
      // `wire/` stays excluded below rather than by narrowing this, because a
      // narrower glob would also stop reporting a future subdirectory of
      // hand-written modules.
      //
      // `node_modules/**` must not be dropped: this list feeds tinyglobby's
