...
export function resolveFieldValue(
  ...
  if (sectionKey === 'poll_interval' && field.key === 'poll_interval') {
    return extractYAMLValue(sections[sectionKey] || '', 'poll_interval')
      || extractTopLevelValue(configText, 'poll_interval') || field.default || '';
  }
  ...
  if (val !== undefined && val !== null) {
    ...
    if (field.type === 'bool') return String(!!val);
...
export function renderFieldsSection(
  ...
  if (hasEnableKey) {
    ...
    if (!isEnabled) content.style.display = 'none';
    const toggleInput = toggle.querySelector('input') as HTMLInputElement | null;
    if (toggleInput) toggleInput.addEventListener('change', () => {
...
function wireShowWhen(
  ...
  for (const field of schema.fields) {
    ...
    if (!depKey || depVal === undefined) continue;
    ...
    if (!depInput || !target) continue;
    const update = (): void => {
      const current = depInput.type === 'checkbox'
        ? String(depInput.checked) : depInput.value;
...
function wireRequires(
  ...
  for (const field of schema.fields) {
    ...
    if (!depInput || !targetEl) continue;
    const targetInput = targetEl.querySelector('input') as HTMLInputElement | null;
  ...
  for (const field of schema.fields) {
    ...
    if (field.group && !seenGroups.has(field.group)) {
      ...
      if (field.type === 'bool') {
        ...
        const value = resolveFieldValue(
          schema.key, field, sections, pc, configText);
        ...
        if (!isOn) groupContent.style.display = 'none';
        const grpInput = toggle.querySelector('input') as HTMLInputElement | null;
        if (grpInput) grpInput.addEventListener('change', () => {
    ...
    if (field.group) {
      const groupEl = container.querySelector(
        `#${CSS.escape(`cfg-group-${field.group}`)}`);
      if (groupEl) { groupEl.appendChild(fieldEl); continue; }
    }
...
export function renderField(id: string, field: SchemaField, value: string): HTMLElement {
  ...
  return cfgField(id, field.label, inputType, value,
    field.placeholder || field.default || '', field.help);
}
...
export function renderListSection(
  ...
  const sec = el('div', { className: 'cfg-section' });
  sec.appendChild(
    el('div', { className: 'cfg-title' }, schema.title));
  ...
  function addItem(value: string): void {
    ...
    const inp = el('input', {
      ...
      placeholder: (fieldSchema && fieldSchema.placeholder) || ''
    });
...
export function renderRawSection(name: string, raw: string): HTMLElement {
  const sec = el('div', { className: 'cfg-section' });
  sec.appendChild(
    el('div', { className: 'cfg-title' }, prettyLabel(name)));
  const ta = el('textarea', {
    id: `section-${name}`, className: 'cfg-textarea',
...
function cfgSecretField(id: string, label: string, value: string): HTMLElement {
  ...
  const inp = el('input', {
    id, type: 'text', className: 'cfg-masked',
  ...
  const toggle = el('button', {
    type: 'button', className: 'cfg-reveal',
  ...
  return el('div', { className: 'cfg-field' },
    el('label', { for: id }, label), container);
}
