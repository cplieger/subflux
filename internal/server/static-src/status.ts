  ...
  if (providers && providers.enabled) {
    ...
    const totalEnabled = cfg
      ? Object.entries(cfg.providers || {})
        .filter(([name, enabled]) => enabled && name !== EMBEDDED_PROVIDER).length
      : 0;
    const timedOut = providers.providers
      ? Object.entries(providers.providers)
        .filter(([name, p]) => p.timed_out && name !== EMBEDDED_PROVIDER).length
      : 0;
    if (totalEnabled > 0) {
...
function updateStatusButton(
  ...
  const hasPersistent = hasAlerts
    && alerts.some((a: Alert) => a.kind === 'persistent');
  const isActive = Array.isArray(activities)
    && activities.some((a: Activity) => !a.done);

  const statusEl = document.getElementById('statusIcon');
  const statusLabel = btn.querySelector('.nav-label');
  if (statusEl) {
    if (hasAlerts && (alerts.some((a: Alert) => a.level === 'error')
        || hasPersistent)) {
      ...
      statusEl.replaceChildren(icon('warning'));
      if (statusLabel) statusLabel.textContent = 'Warning';
    } else if (isActive) {
...
function processActivitySideEffects(
  ...
  if (toastedActivities.size === 0 && Array.isArray(activities)) {
    for (const a of activities) {
      if (a.done) toastedActivities.add(a.id);
    }
  }
...
function buildActivityItem(a: Activity): HTMLElement {
  ...
  if (a.done) {
    statusIcon = el('span', { className: 'act-icon act-done' },
      icon('check'));
  } else if (a.queued) {
    statusIcon = el('span', { className: 'act-icon act-queued' },
      icon('hourglass'));
  } else {
    statusIcon = el('span', { className: 'act-icon act-active' },
      el('span', { className: 'spinner' }));
  }

  const titleText = a.source === 'scheduled'
    ? 'Scheduled search' : 'Manual search';

  let timerSpan: HTMLElement | null;
  if (a.done && a.ended_at) {
    timerSpan = el('span', { className: 'live-timer' },
      ` \u00B7 ${formatDuration(
...
function buildPopupContent(
  ...
  if (!isActive && stats && stats.last_scan) {
    ...
    if (stats.scan_interval_seconds > 0 && lastDone
        && lastDone.ended_at) {
      const nextMs = new Date(lastDone.ended_at).getTime()
        + stats.scan_interval_seconds * 1000;
...
function dismissActivity(id: string): void {
  ...
  if (item) {
    const btn = item.querySelector('.close-btn') as HTMLButtonElement | null;
    if (btn) {
      btn.disabled = true;
...
export function updateLiveTimers(): void {
  const now = new Date();
  document.querySelectorAll('.live-timer[data-started]').forEach(
    (timer: Element) => {
      timer.textContent = ` \u00B7 ${formatDuration(
        new Date(timer.getAttribute('data-started') || ''), now)}`;
    });
}
