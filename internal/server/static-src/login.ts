...
async function detectAuthMethods(): Promise<void> {
  try {
    const res = await fetch('/api/auth/oidc', { method: 'HEAD', redirect: 'manual', signal: AbortSignal.timeout(10_000) });
    if (res.status === 200 || res.type === 'opaqueredirect' || res.status === 302) {
      show($('oidcBtn'));
      show($('authDivider'));
    }
  } catch { /* OIDC not available */ }
  ...
  $('oidcBtn')?.addEventListener('click', () => { window.location.href = '/api/auth/oidc'; });
...
async function startConditionalUI(): Promise<void> {
  ...
  try {
    const options = await apiPost<WebAuthnOptions>('/api/auth/webauthn/login/begin?mediation=conditional');
    if (!options) return;
    ...
    if (pk.allowCredentials) {
      for (const cred of pk.allowCredentials) cred.id = base64urlToBuffer(cred.id as unknown as string);
    }
...
async function passkeyLogin(): Promise<void> {
  ...
  try {
    ...
    if (!options) { showError('loginError', 'Failed to start passkey login'); return; }
    webauthnSessionToken = options['session_token'] || '';
    const pk = options['publicKey'];
    if (!pk) return;
    pk.challenge = base64urlToBuffer(pk.challenge as unknown as string);
...
async function finishWebAuthnLogin(credential: PublicKeyCredential, sessionToken: string): Promise<void> {
  ...
  if (!res.ok) {
    ...
    if (data['signal'] === 'unknown_credential') {
      showError('loginError', 'This passkey is not recognized. Please delete it from your authenticator and try again.');
      return;
    }
    if (data['code'] === ErrorCode.WebAuthnSessionInvalid || data['error'] === 'invalid or expired session') {
      startConditionalUI();
      return;
    }
...
const LOGIN_ERROR_MAP: ReadonlyArray<{code: ErrorCode; msg: string | ((res: ApiResult<LoginResponse>) => string)}> = [
  { code: ErrorCode.RateLimited, msg: (res) => {
    const retry = res.headers?.get('Retry-After');
    if (retry) return `Too many login attempts; try again in ${retry} seconds.`;
    return 'Too many login attempts. Please wait and try again.';
  }},
  { code: ErrorCode.AuthAccountDisabled, msg: 'This account has been disabled.' },
  { code: ErrorCode.AuthAccountNotSetup, msg: 'Account setup is incomplete. Contact your administrator.' },
...
function loginErrorMessage(res: ApiResult<LoginResponse>): string {
  ...
  if (entry) return typeof entry.msg === 'function' ? entry.msg(res) : entry.msg;
...
function wireTOTPForm(): void {
  ...
  form.addEventListener('submit', async (e: Event) => {
    ...
    const res = await apiPostRaw<TOTPResponse>('/api/auth/totp', {
      code, pending_token: pendingToken
    });
    if (!res.ok) {
