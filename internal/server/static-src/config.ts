...
import {
  buildLanguagesSection, serializeLanguagesFromForm
} from './config-languages.js';
import {
  renderProvidersSection, renderEmbeddedSection, genProviders
} from './config-providers.js';
import {
  fieldId, renderFieldsSection, renderListSection, renderRawSection
...
const YAML_ERROR_PATTERNS: ReadonlyArray<{ match: string; message: string }> = [
  ...
  { match: 'found character that cannot start', message: 'invalid character, try quoting the value' },
];
...
function friendlyConfigError(raw: string): string {
  ...
  const yamlLine = msg.match(/^yaml:\s*line\s*(\d+)/i);
  if (yamlLine) {
    const line = yamlLine[1];
    const rule = YAML_ERROR_PATTERNS.find(r => msg.includes(r.match));
    if (rule) return `Syntax error on line ${line}: ${rule.message}`;
...
const CONFIG_SAVE_ERRORS: ReadonlyArray<{ code: ErrorCode; msg: string }> = [
  { code: ErrorCode.ConfigUnreachableArr, msg: 'Sonarr/Radarr unreachable. Check the URL and API key.' },
  { code: ErrorCode.ConfigTooLarge, msg: 'Configuration is too large. Remove unused entries and try again.' },
  { code: ErrorCode.ConfigReloadFailed, msg: 'Configuration saved but not applied. Check server logs for details.' },
];
...
function renderConfigForm(): void {
  ...
  if (isFirstSetup) markRequiredFields();
}
...
function markRequiredFields(): void {
  ...
  for (const schema of configSchema) {
    if (!schema.required_group || !schema.fields) continue;
    const grp = schema.required_group;
    if (!(grp in groupFilled)) groupFilled[grp] = false;
    const allFilled = schema.fields
      .filter((f: SchemaField) => f.required)
      .every((f: SchemaField) => {
        const inp = body.querySelector(`#${CSS.escape(fieldId(schema.key, f.key))}`) as HTMLInputElement | null;
        if (!inp) return false;
        // Secret fields with a placeholder have a redacted value on the server.
        if (f.secret && inp.placeholder === '****') return true;
        return (inp.value || '').trim() !== '';
      });
    if (allFilled) groupFilled[grp] = true;
  }
  ...
  for (const schema of configSchema) {
    ...
    for (const field of schema.fields) {
      ...
      const inp = body.querySelector(`#${CSS.escape(fieldId(schema.key, field.key))}`) as HTMLInputElement | null;
      if (!inp) continue;
      ...
      if (!inp.dataset['requiredWired']) {
        ...
        inp.addEventListener('input', () => updateFieldValidation(inp, field));
      }
    }
  }
}
  ...
  for (const schema of configSchema) {
    if (schema.type === 'providers') {
      ...
      const f = document.getElementById(
        fieldId(schema.key, 'poll_interval')) as HTMLInputElement | null;
...
function genFields(schema: ConfigSchema): string {
  ...
  if (schema.enable_key) {
    const t = document.getElementById(
      fieldId(schema.key, schema.enable_key)) as HTMLInputElement | null;
    lines.push(`  ${schema.enable_key}: ${t ? String(t.checked) : 'true'}`);
  }
  for (const field of schema.fields) {
    if (field.key === schema.enable_key) continue;
...
function genScoring(schema: ConfigSchema): string {
  ...
  for (const field of schema.fields) {
    const f = document.getElementById(
      fieldId(schema.key, field.key)) as HTMLInputElement | null;
...
function genList(schema: ConfigSchema): string {
  ...
  if (listEl) {
    for (const inp of Array.from(
      listEl.querySelectorAll('input[type="text"]') as NodeListOf<HTMLInputElement>
    )) {
