...
describe('bus', () => {
  describe('on/emit', () => {
    ...
    for (const tc of cases) {
      it(tc.name, () => {
        ...
        const unsub = on(tc.event, (...args: unknown[]) => { received.push(args); });
  ...
  it('unsubscribe removes listener', () => {
    let count = 0;
    const unsub = on(BusEvent.LoadHistory, () => { count++; });
  ...
  it('listener error does not prevent other listeners', () => {
    const calls: number[] = [];
    const unsub1 = on(BusEvent.LoadHistory, () => { throw new Error('boom'); });
    const unsub2 = on(BusEvent.LoadHistory, () => calls.push(2));
    emit(BusEvent.LoadHistory);
    unsub1();
    unsub2();
