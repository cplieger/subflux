...
func TestHandleUI_serves_static_file(t *testing.T) {
	...
	req := httptest.NewRequestWithContext(context.Background(),
		http.MethodGet, "/style.css", http.NoBody)
	rec := httptest.NewRecorder()
	s.handleUI(rec, req)

	if rec.Code != http.StatusOK {
		t.Errorf("handleUI(/style.css) status = %d, want %d", rec.Code, http.StatusOK)
	}
	if ct := rec.Header().Get("Content-Type"); !strings.Contains(ct, "css") {
		t.Errorf("handleUI(/style.css) Content-Type = %q, want css", ct)
	}
}
