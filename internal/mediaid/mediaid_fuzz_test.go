...
func FuzzMovie(f *testing.F) {
	f.Add(100, "tt1234567")
	f.Add(0, "tt0000001")
	f.Add(0, "")
	f.Add(-5, "")
	f.Fuzz(func(t *testing.T, tmdbID int, imdbID string) {
		result := Movie(tmdbID, imdbID)
...
// Episode appends. A constructed movie ID ("tmdb-<n>") must never carry
// this suffix; a bare "-s" inside a garbage IMDB fallback is not an episode
// marker.
var episodeIDSuffixRe = regexp.MustCompile(`-s\d+e\d+$`)

// FuzzBuild cross-checks the dispatcher against the individual ID
...
// FuzzValidPrefix verifies ValidPrefix never panics and pins the
...
func FuzzValidPrefix(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, prefix, suffix string) {
		if ValidPrefix(prefix) && !ValidPrefix(prefix+suffix) {
			t.Errorf("ValidPrefix(%q) = true but ValidPrefix(%q) = false; appending a suffix must not invalidate an accepted prefix", prefix, prefix+suffix)
		}
	})
}
