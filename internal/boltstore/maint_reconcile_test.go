...
func TestReconcileState_allPresentIsNoop(t *testing.T) {
	...
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "tt1", "fr",
		subflux.ProviderNameOpenSubtitles, "R", sub, video, 100, false)); err != nil {
	...
	if rows := readTripleRows(t, db, subflux.MediaTypeMovie, "tt1", "fr"); len(rows) != 1 {
...
func TestReconcileState_videoGoneDeletesFanout(t *testing.T) {
	...
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeEpisode, "ep1", "fr",
		subflux.ProviderNameOpenSubtitles, "Auto.FR", subFR, video, 80, false)); err != nil {
		t.Fatalf("SaveDownload(auto fr): %v", err)
	}
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeEpisode, "ep1", "fr",
		subflux.ProviderNameSubDL, "Manual.FR", subFRm, video, 50, true)); err != nil {
		t.Fatalf("SaveDownload(manual fr): %v", err)
	}
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeEpisode, "ep1", "en",
		subflux.ProviderNameOpenSubtitles, "Auto.EN", subEN, video, 90, false)); err != nil {
		t.Fatalf("SaveDownload(auto en): %v", err)
	}
	seedFile(t, db, subflux.MediaTypeEpisode, "ep1", "fr", subFR)
	seedScan(t, db, subflux.MediaTypeEpisode, "ep1")
	// Backoff on both affected triples (a provider the saves did not clear).
	seedAttempt(t, db, subflux.MediaTypeEpisode, "ep1", "fr", subflux.ProviderNameGestdown)
	seedAttempt(t, db, subflux.MediaTypeEpisode, "ep1", "en", subflux.ProviderNameGestdown)
...
func TestReconcileState_siblingPresentDeletesOnlyMissingRow(t *testing.T) {
	...
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "tt1", "fr",
		subflux.ProviderNameOpenSubtitles, "Auto", autoSub, video, 80, false)); err != nil {
		t.Fatalf("SaveDownload(auto): %v", err)
	}
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "tt1", "fr",
		subflux.ProviderNameSubDL, "Manual", manualSub, video, 50, true)); err != nil {
		t.Fatalf("SaveDownload(manual present): %v", err)
	}
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "tt1", "fr",
		subflux.ProviderNameSubDL, "ManualGone", manualGone, video, 50, true)); err != nil {
	...
	rows := readTripleRows(t, db, subflux.MediaTypeMovie, "tt1", "fr")
	if len(rows) != 1 {
	...
	locked, err := db.IsManuallyLocked(ctx, subflux.ManualLockKey{MediaType: subflux.MediaTypeMovie, MediaID: "tt1", Language: "fr", Variant: subflux.VariantStandard})
	if err != nil {
...
func TestReconcileState_allSubsGoneResetsAutoDeletesManual(t *testing.T) {
	...
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "tt1", "fr",
		subflux.ProviderNameOpenSubtitles, "Auto", autoSub, video, 100, false)); err != nil {
		t.Fatalf("SaveDownload(auto): %v", err)
	}
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "tt1", "fr",
		subflux.ProviderNameSubDL, "Manual", manualSub, video, 200, true)); err != nil {
		t.Fatalf("SaveDownload(manual): %v", err)
	}
	seedAttempt(t, db, subflux.MediaTypeMovie, "tt1", "fr", subflux.ProviderNameGestdown)

	// Capture the auto row's original media_imported to prove the bump.
	autoBefore, _ := partitionRows(readTripleRows(t, db, subflux.MediaTypeMovie, "tt1", "fr"))
	if len(autoBefore) != 1 {
...
func TestReconcileState_allSubsGoneAutoOnlyReset(t *testing.T) {
	...
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "tt1", "fr",
		subflux.ProviderNameOpenSubtitles, "RelA", sub1, video, 100, false)); err != nil {
		t.Fatalf("SaveDownload(auto A): %v", err)
	}
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "tt1", "fr",
		subflux.ProviderNameSubDL, "RelB", sub2, video, 110, true)); err != nil {
		t.Fatalf("SaveDownload(manual B): %v", err)
	}
	if err := db.ClearManualLock(ctx, subflux.ManualLockKey{MediaType: subflux.MediaTypeMovie, MediaID: "tt1", Language: "fr", Variant: subflux.VariantStandard}); err != nil {
		t.Fatalf("ClearManualLock: %v", err)
	}
	// Now two auto rows exist for the triple.
	if auto, _ := partitionRows(readTripleRows(t, db, subflux.MediaTypeMovie, "tt1", "fr")); len(auto) != 2 {
...
func TestReconcileState_unrelatedMediaUntouched(t *testing.T) {
	...
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "ttDel", "fr",
		subflux.ProviderNameOpenSubtitles, "Del", delSub, delVideo, 100, false)); err != nil {
	...
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "ttKeep", "en",
		subflux.ProviderNameOpenSubtitles, "Keep", keepSub, keepVideo, 100, false)); err != nil {
		t.Fatalf("SaveDownload(keep): %v", err)
	}
	seedAttempt(t, db, subflux.MediaTypeMovie, "ttKeep", "en", subflux.ProviderNameGestdown)
	seedFile(t, db, subflux.MediaTypeMovie, "ttKeep", "en", keepSub)
	...
	// Doomed item gone.
	if rows := readTripleRows(t, db, subflux.MediaTypeMovie, "ttDel", "fr"); len(rows) != 0 {
...
func TestReconcileState_idempotent(t *testing.T) {
	...
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "ttGone", "fr",
		subflux.ProviderNameOpenSubtitles, "Gone", subGone, videoGone, 100, false)); err != nil {
	...
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "ttReset", "fr",
		subflux.ProviderNameOpenSubtitles, "Reset", subReset, videoReset, 100, false)); err != nil {
	...
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "ttSib", "fr",
		subflux.ProviderNameOpenSubtitles, "SibAuto", sibAuto, videoSib, 80, false)); err != nil {
		t.Fatalf("SaveDownload(sib auto): %v", err)
	}
	if err := db.SaveDownload(ctx, dlRec(subflux.MediaTypeMovie, "ttSib", "fr",
		subflux.ProviderNameSubDL, "SibManual", sibManual, videoSib, 50, true)); err != nil {
	...
	gone1 := readTripleRows(t, db, subflux.MediaTypeMovie, "ttGone", "fr")
	reset1 := readTripleRows(t, db, subflux.MediaTypeMovie, "ttReset", "fr")
	sib1 := readTripleRows(t, db, subflux.MediaTypeMovie, "ttSib", "fr")
	...
	gone2 := readTripleRows(t, db, subflux.MediaTypeMovie, "ttGone", "fr")
	reset2 := readTripleRows(t, db, subflux.MediaTypeMovie, "ttReset", "fr")
	sib2 := readTripleRows(t, db, subflux.MediaTypeMovie, "ttSib", "fr")
