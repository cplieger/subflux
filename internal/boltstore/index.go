...
//
// subtitle_state's primary key is the bare be64(id) surrogate, but its value
// is self-contained (the quad lives in stateRec; see codec.go), so every index
// key — including ix_state_quad — derives from the record alone and the spec
// is as static as the others. A given surrogate id belongs to exactly one quad
// for its whole life, so the read-old-delete-stale step inside kv always
// rebuilds the same quad key it originally wrote.
...
// by default, which wastes half a page under tail-append workloads; 0.9 is
// the value etcd uses for its own sequentially-filled keys bucket. Buckets
// with random insert order (text-keyed primaries, ix_state_quad,
// ix_state_video) keep the default — a high fill percent there causes poor
// page utilization (README caveat). FillPercent is a per-transaction bucket
// field, so the write chokepoints set it on each write tx.
const appendFillPercent = 0.9

// setAppendFill raises FillPercent on the named bucket for the duration of
// this transaction. A missing bucket is skipped rather than dereferenced: the
// kv write helper that follows reports it as a clean "bucket not found" error
// (bucket bootstrap is Open's job), which beats a nil-pointer panic here.
func setAppendFill(tx *bolt.Tx, bucket string) {
	if b := tx.Bucket([]byte(bucket)); b != nil {
		b.FillPercent = appendFillPercent
	}
}
...
// --- ix_state_quad projected-value codec ---
//
// ix_state_quad carries a small projection of (manual, score, provider) as
...
// stateIndexes declares the three subtitle_state secondary indexes, all
// derived from the self-contained record:
//
//   - ix_state_quad     (mt 0x00 mid 0x00 lang 0x00 variant 0x00 be64(id))
//     carrying the (manual, score, provider) projection
//   - ix_state_imported (be64(media_imported) 0x00 be64(id)), existence-only
//   - ix_state_video    (video_path 0x00 be64(id)), existence-only
//
// Every key derives entirely from the record, so on a reset that changes
// media_imported (or a row whose video_path changes) kv's
// read-old-delete-stale step removes the old entry and adds the new one in
// the same tx.
func stateIndexes() []kv.IndexSpec[stateRec] {
	return []kv.IndexSpec[stateRec]{
		{
			Bucket: bucketIxStateQuad,
			Key: func(_ []byte, v *stateRec) []byte {
				return stateQuadKey(v.MediaType, v.MediaID, v.Language, v.Variant, v.ID)
			},
...
// and maintains the three state indexes and the downloads counter, all in tx.
// rec must carry its full quad (the ix_state_quad key is derived from it) and
// an allocated ID (via kv.NextID on the subtitle_state bucket); an incomplete
// quad is refused so a malformed index key can never be written.
//
// The subtitle_state primary is keyed by a monotonically increasing sequence
// and ix_state_imported by an ever-increasing timestamp, so both fill their
// pages tail-first; the raised FillPercent packs those appends tighter than
// the 50% split default (the etcd keys-bucket setting).
func putState(tx *bolt.Tx, rec *stateRec) error {
	if rec.MediaType == "" || rec.MediaID == "" || rec.Language == "" || rec.Variant == "" {
		return fmt.Errorf("boltstore: putState: incomplete quad (%q/%q/%q/%q)",
			rec.MediaType, rec.MediaID, rec.Language, rec.Variant)
	}
	setAppendFill(tx, bucketSubtitleState)
	setAppendFill(tx, bucketIxStateImported)
...
// absent key. When a row is actually removed, the file's stored sync offset
// (sync_offsets, keyed by the bare path — the key's last component) is deleted
// in the same transaction, so sync_offsets never accumulates stale keys for
// subtitles that no longer exist; a re-created file at the same path starts at
// offset 0 rather than inheriting the old file's offset.
func deleteSubtitleFile(tx *bolt.Tx, key []byte) (existed bool, err error) {
	existed, err = kv.DeleteIndexed[fileRec](tx, bucketSubtitleFiles, key, nil, fileCounters())
	if err != nil || !existed {
		return existed, err
	}
	if parts := kv.Split(key); len(parts) >= 6 {
		if b := tx.Bucket([]byte(bucketSyncOffsets)); b != nil {
			if derr := b.Delete([]byte(parts[5])); derr != nil {
				return existed, fmt.Errorf("boltstore: delete sync offset for removed subtitle: %w", derr)
			}
		}
	}
	return existed, nil
