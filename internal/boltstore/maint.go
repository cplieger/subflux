...
// ReconcileState so both use the same orphan-cleanup fan-out.
...
// tripleRef identifies a (media_type, media_id, language) backoff owner.
...
//     ix_state_quad / ix_state_imported / ix_state_video entries and
//     decrements the downloads counter in the same tx),
//   - clears every affected language triple's search_attempts backoff via
//     clearTripleBackoff,
...
func (d *DB) deletePathsBatch(paths []string) ([]string, error) {
	...
	err := d.db.Update(func(tx *bolt.Tx) error {
		...
		affectedTriples := make(map[tripleRef]struct{})
		affectedMedia := make(map[mediaRef]struct{})

		for _, p := range paths {
			batchPaths, perr := d.deleteStateRowsForPath(tx, sb, p, affectedTriples, affectedMedia)
			if perr != nil {
				return perr
			}
			subPaths = append(subPaths, batchPaths...)
		...
		// Success/removal clears the affected languages' adaptive backoff.
...
// chokepoint, and records the affected language triples (for backoff cleanup)
// and media items in the shared sets. Each row is decoded once (the record is
// self-contained) to capture its subtitle path (non-empty only, for the
// caller's disk cleanup, matching the old `... AND path != ”` projection) and
...
func (d *DB) deleteStateRowsForPath(tx *bolt.Tx, sb *bolt.Bucket, videoPath string, affectedTriples map[tripleRef]struct{}, affectedMedia map[mediaRef]struct{}) ([]string, error) {
	...
	for _, id := range ids {
		...
		if raw == nil {
			continue // index/primary drift: nothing to delete
		}
		var sr stateRec
		if derr := kv.Decode(raw, &sr); derr != nil {
			return nil, fmt.Errorf("boltstore: decode subtitle_state id=%d: %w", id, derr)
		}
		if sr.Path != "" {
		...
		affectedMedia[mediaRef{mt: sr.MediaType, mid: sr.MediaID}] = struct{}{}
	}
	return subPaths, nil
}

// clearBackoffForTriples clears the adaptive backoff of every language triple
// in the set through the clearTripleBackoff chokepoint, so the attempts
// counter stays consistent.
func clearBackoffForTriples(tx *bolt.Tx, triples map[tripleRef]struct{}) error {
...
// item, by checking ix_state_quad for any entry under mediaPrefix(mt, mid).
...
//     row is cleared. This short-circuits the
...
//
// All deletes route through the deleteAttempt chokepoint, so the attempts
// counter stays consistent. The whole operation runs in
...
// deleteAttempt chokepoint so the attempts counter stays consistent. It walks
...
// matching the old loadEntries `WHERE video_path != ”`. The record is
// self-contained, so this is one pass over the primary bucket with no index
// walk. An undecodable record is tolerate-skipped (subtitle_state is a derived
// bucket the next scan rebuilds). Values are copied out so the filesystem
...
func (d *DB) loadReconcileEntries() ([]reconcileEntry, error) {
	...
	err := d.db.View(func(tx *bolt.Tx) error {
		...
		return sb.ForEach(func(k, v []byte) error {
			var sr stateRec
			skip, derr := decodeRecord(bucketDecodeMode(bucketSubtitleState), bucketSubtitleState, k, v, &sr)
			if derr != nil {
				return derr
			}
			...
			entries = append(entries, reconcileEntry{
				id:        sr.ID,
				quad:      stateQuadInfo{mt: sr.MediaType, mid: sr.MediaID, lang: sr.Language, variant: sr.Variant},
				subPath:   sr.Path,
...
func (d *DB) classifyReconcileEntries(entries []reconcileEntry) (
	...
	subMissing = make(map[stateQuadInfo][]reconcileEntry)
	subPresent = make(map[stateQuadInfo]bool)
	...
	for i := range entries {
		...
		case reconcileSubMissing:
			subMissing[e.quad] = append(subMissing[e.quad], e)
		case reconcileSubPresent:
			subPresent[e.quad] = true
		case reconcileSkip:
...
func (d *DB) reconcileMissingGroups(
	...
	keys := make([]stateQuadInfo, 0, len(subMissing))
...
// quads in a single all-or-none Update, returning how many quads were reset
// (the all-subtitles-gone branch). Per quad:
//
...
func (d *DB) reconcileResetBatch(
	...
	err := d.db.Update(func(tx *bolt.Tx) error {
		...
		for _, q := range keys {
			reset, rerr := reconcileQuad(tx, sb, q, subMissing[q], subPresent[q])
			if rerr != nil {
				return rerr
			}
			if reset {
...
func reconcileQuad(tx *bolt.Tx, sb *bolt.Bucket, q stateQuadInfo, missing []reconcileEntry, siblingPresent bool) (bool, error) {
	...
	if err := reconcileResetQuad(tx, sb, q, missing); err != nil {
		return false, err
	}
	return true, nil
}

// deleteMissingRows deletes only the missing-subtitle rows of a quad (the
...
// reconcileResetQuad applies the all-subtitles-gone branch for a single quad
// inside an open Update: delete each manual row, reset each auto row in place,
// then clear the language's backoff. An auto row is reset by clearing
...
// provider rewrites the ix_state_quad projection in the same tx. Backoff is
// cleared for the whole language triple (search_attempts has no variant
// dimension); clearing it more than once per language in a batch is an
// idempotent no-op.
func reconcileResetQuad(tx *bolt.Tx, sb *bolt.Bucket, q stateQuadInfo, missing []reconcileEntry) error {
	now := time.Now()
	for _, e := range missing {
		if e.manual {
			if _, derr := deleteState(tx, e.id); derr != nil {
		...
		if perr := putState(tx, &sr); perr != nil {
			return perr
		}
	}
	// Clear the language's adaptive backoff so the next scan re-searches.
	return clearTripleBackoff(tx, q.mt, q.mid, q.lang)
}
