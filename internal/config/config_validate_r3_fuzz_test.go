...
// FuzzValidateBackup exercises validateBackup with arbitrary inputs,
// ensuring it never panics and that disabled configs always pass.
...
// FuzzValidateLogging exercises validateLogging with arbitrary level/format strings.
...
// FuzzValidateScoreRange exercises validateScoreRange boundaries.
func FuzzValidateScoreRange(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, value int, field string) {
		...
		// If error, it should be a *ValidationError.
		if err != nil {
