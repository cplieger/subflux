...
// 9 of the 37 values the configuration offers.
type Cfg interface {
	// Scores must return non-zero values; zero scores disable all attribute matching.
	...
	// An empty slice means no providers will be searched for this target.
	ProvidersForTarget(t *subflux.SubtitleTarget, allProviders []subflux.ProviderID) []subflux.ProviderID
	// MinScoreForTarget returns 0 to accept any score.
	MinScoreForTarget(t *subflux.SubtitleTarget, mediaType subflux.MediaType) int
	// ProviderPriority returns 0 for unknown providers (lowest priority = tried last in tiebreakers).
