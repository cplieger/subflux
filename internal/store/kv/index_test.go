...
//
// Each step is a named subtest over the SAME database, in order: the steps are
// a state machine, not independent cases, so they must not run in parallel.
// Naming them means a failure reports which transition broke instead of a line
// number, and one broken step no longer stops the walk before the later
// transitions are exercised at all.
func TestPutDeleteIndexed_maintainsIndexAndCounter(t *testing.T) {
	db := openTestDB(t)
	mustCreateBuckets(t, db, bktPrimary, bktIndex, bktMeta)

	t.Run("insert k1 indexes it and counts it", func(t *testing.T) {
		put(t, db, "k1", idxRec{Tag: "a", N: 1})
		if c := readCount(t, db); c != 1 {
			t.Errorf("counter = %d, want 1", c)
		}
		entries := indexEntries(t, db)
		if len(entries) != 1 {
			t.Fatalf("%d index entries, want 1", len(entries))
		}
	...
	t.Run("re-tagging k1 replaces its index entry and leaves the count", func(t *testing.T) {
		put(t, db, "k1", idxRec{Tag: "b", N: 2})
		if c := readCount(t, db); c != 1 {
			t.Errorf("counter = %d, want 1 (an update is not an insert)", c)
		}
		entries := indexEntries(t, db)
		if len(entries) != 1 {
			t.Fatalf("%d index entries, want 1", len(entries))
	...
	t.Run("a second row sharing a tag gets its own entry", func(t *testing.T) {
		put(t, db, "k2", idxRec{Tag: "b", N: 3})
		if c := readCount(t, db); c != 2 {
			t.Errorf("counter = %d, want 2", c)
		}
		if entries := indexEntries(t, db); len(entries) != 2 {
			t.Errorf("%d index entries, want 2", len(entries))
		}
	})

	t.Run("deleting k1 leaves only k2 indexed", func(t *testing.T) {
		if !del(t, db, "k1") {
	...
	t.Run("deleting a missing key changes nothing", func(t *testing.T) {
		if del(t, db, "missing") {
			t.Error("DeleteIndexed(missing) reported existed = true, want false")
		}
		if c := readCount(t, db); c != 1 {
			t.Errorf("counter = %d, want 1 (unchanged)", c)
		}
	})

	t.Run("emptying the store clamps the counter at zero", func(t *testing.T) {
		del(t, db, "k2")
		del(t, db, "k2") // a second delete must not underflow the counter
		if c := readCount(t, db); c != 0 {
			t.Errorf("counter = %d, want 0", c)
		}
		if entries := indexEntries(t, db); len(entries) != 0 {
			t.Errorf("%d index entries, want 0", len(entries))
		}
	})
}
