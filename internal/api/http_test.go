...
func TestRequestLogger_attaches_id_and_echoes_header(t *testing.T) {
	t.Parallel()
	...
	if capturedID == "" {
		t.Error("RequestLogger did not attach a request id")
	}
	if got := rec.Header().Get(HeaderXRequestID); got != capturedID {
		t.Errorf("X-Request-ID header = %q, want context id %q", got, capturedID)
	}
}

func TestRequestLogger_reuses_inbound_valid_id(t *testing.T) {
	t.Parallel()
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/x", http.NoBody)
	req.Header.Set(HeaderXRequestID, "valid-inbound-id")
	mw.ServeHTTP(rec, req)

	if capturedID != "valid-inbound-id" {
		t.Errorf("inbound id should be reused, got %q", capturedID)
	}
}

func TestRequestLogger_rejects_malformed_inbound_id(t *testing.T) {
	t.Parallel()
	...
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, "/x", http.NoBody)
	// Header injection attempt: newline + log forging.
	req.Header.Set(HeaderXRequestID, "evil\ninjected log line")
	mw.ServeHTTP(rec, req)

	if capturedID == "evil\ninjected log line" {
		t.Error("RequestLogger leaked a malformed inbound id")
	}
	if capturedID == "" {
