...
save_config() {
    ...
    [ -n "$ORIGINAL_CONFIG" ] || { printf '%bERROR: empty config%b\n' "$RED" "$NC"; exit 1; }
...
test_health() {
    log "=== Health & Basics ==="
    api_get "/health";  assert_status 200 "GET /health"
    api_get "/metrics"; assert_status 200 "GET /metrics"
...
test_auth() {
    ...
    # Setup status
    local setup
    setup=$(api_get "/api/auth/setup"); sync_status
    ...
    local me
    me=$(api_get "/api/auth/me"); sync_status
    if [ "$HTTP_STATUS" = "200" ]; then
    ...
    api_get "/api/auth/passkeys"; sync_status
    ...
    api_get "/api/auth/webauthn/signal-data"; sync_status
    [ "$HTTP_STATUS" = "200" ] || [ "$HTTP_STATUS" = "400" ] || [ "$HTTP_STATUS" = "401" ] \
...
test_config() {
    ...
    api_put "/api/config" "$cfg"; assert_status 200 "PUT /api/config (unchanged)"
    ...
    reset_cfg=$(api_get "/api/config"); sync_status
    [ "$reset_cfg" != "$cfg" ] && pass "Reset changed config" || skip "Reset same (already default)"

    api_put "/api/config" "$cfg" >/dev/null; sleep 1
}
...
test_providers() {
    ...
    api_get "/api/providers/timeout"; assert_status 200 "GET /api/providers/timeout"
    api_post "/api/providers/timeout/reset"; assert_status 200 "POST /api/providers/timeout/reset"
}
...
test_media_browser() {
    ...
    series=$(api_get "/api/media/series"); sync_status
    ...
    if [ -n "$first_id" ] && [ "$first_id" != "null" ]; then
        episodes=$(api_get "/api/media/series/${first_id}/episodes"); sync_status
    ...
    movies=$(api_get "/api/media/movies"); sync_status
...
test_coverage() {
    log "=== Coverage ==="
    api_get "/api/coverage/series"; assert_status 200 "GET /api/coverage/series"
    api_get "/api/coverage/movies"; assert_status 200 "GET /api/coverage/movies"
    api_get "/api/coverage/scan-state"; assert_status 200 "GET /api/coverage/scan-state"

    local sid mid
    sid=$(api_get "/api/media/series" | jq -r '.[0].id // empty' 2>/dev/null)
    sync_status
    if [ -n "$sid" ]; then
        api_get "/api/coverage/series/${sid}"; assert_status 200 "Coverage detail series/$sid"
    else
...
test_state() {
    log "=== State & History ==="
    local stats
    stats=$(api_get "/api/state/stats"); sync_status
    ...
    api_get "/api/state"; assert_status 200 "GET /api/state"
    api_get "/api/state?type=episode&limit=5"; assert_status 200 "State filter: episode"
    api_get "/api/state?type=movie&limit=5"; assert_status 200 "State filter: movie"
    api_get "/api/state?lang=fr&limit=5"; assert_status 200 "State filter: lang=fr"
    api_get "/api/state?search=test&limit=5"; assert_status 200 "State filter: search"
    api_get "/api/state?limit=5&offset=0"; assert_status 200 "State: pagination"

    api_get "/api/state/ids?type=episode"; assert_status 200 "History IDs: episode"
    api_get "/api/state/ids?type=movie"; assert_status 200 "History IDs: movie"

    # Invalid type for history IDs
    api_get "/api/state/ids?type=invalid"; sync_status
    [ "$HTTP_STATUS" = "400" ] && pass "History IDs: invalid type rejected" \
        || log "History IDs invalid type: HTTP $HTTP_STATUS"

    api_get "/api/activity"; assert_status 200 "GET /api/activity"

    # Dismiss activity (no-op if nothing to dismiss)
    api_delete "/api/activity"; sync_status
    [ "$HTTP_STATUS" = "200" ] || [ "$HTTP_STATUS" = "204" ] \
        && pass "DELETE /api/activity (HTTP $HTTP_STATUS)" \
        || log "Dismiss activity: HTTP $HTTP_STATUS"

    api_get "/api/backoff"; assert_status 200 "GET /api/backoff"
    api_get "/api/backoff/prefix?type=episode&prefix=tvdb-81189-"; assert_status 200 "Backoff prefix: episode"
    api_get "/api/backoff/prefix?type=movie&prefix=tmdb-27205"; assert_status 200 "Backoff prefix: movie"
    api_get "/api/locks"; assert_status 200 "GET /api/locks"
    api_get "/api/alerts"; assert_status 200 "GET /api/alerts"

    # Dismiss alerts
    api_delete "/api/alerts"; sync_status
...
test_files() {
    log "=== File Management ==="
    api_get "/api/files?media_type=episode&media_id=tvdb-"; assert_status 200 "Files: episode prefix"
    api_get "/api/files?media_type=movie&media_id=tmdb-"; assert_status 200 "Files: movie prefix"
...
test_manual_search() {
    ...
    r=$(api_get "/api/search?title=Inception&year=2010&lang=en&type=movie&imdb=tt1375666"); sync_status
    ...
    r=$(api_get "/api/search?title=Breaking+Bad&season=1&episode=1&lang=en&type=episode&imdb=tt0903747&tvdb=81189"); sync_status
    ...
    api_get "/api/search?lang=en&type=movie"; assert_status 200 "Search: no title"
    api_get "/api/search?title=Test&lang=xx&type=movie"; log "Invalid lang: HTTP $HTTP_STATUS"
    api_get "/api/search?title=Test&lang=en&type=invalid"; log "Invalid type: HTTP $HTTP_STATUS"

    api_get "/api/search/targets?orig_lang=en&audio_langs=en"; assert_status 200 "Targets: en audio"
    api_get "/api/search/targets?orig_lang=ja&audio_langs=ja"; assert_status 200 "Targets: ja audio"
    api_get "/api/search/targets?orig_lang=fr&audio_langs=fr"; assert_status 200 "Targets: fr audio"
}
...
test_scoring() {
    ...
    s=$(api_post "/api/score" '{"media_type":"movie","video_release":"Inception.2010.1080p.BluRay.x264-SPARKS","sub_release":"Inception.2010.1080p.BluRay.x264-SPARKS","matched_by":"title"}'); sync_status
    assert_status 200 "Score: exact match"
    assert_json_not_empty "$s" ".score" "Has score"
    assert_json_not_empty "$s" ".tier" "Has tier"

    s=$(api_post "/api/score" '{"media_type":"movie","video_release":"X","sub_release":"X","matched_by":"hash"}'); sync_status
...
test_mock_provider() {
    ...
    apply_mock_config "static" '      result_count: "5"
      score_base: "80"'
    r=$(api_get "/api/search?title=Test+Movie&year=2024&lang=en&type=movie"); sync_status
    ...
    apply_mock_config "static" '      languages: "fr"
      result_count: "3"'
    r=$(api_get "/api/search?title=Test&year=2024&lang=en&type=movie"); sync_status
    ...
    r=$(api_get "/api/search?title=Test&year=2024&lang=fr&type=movie"); sync_status
    count=$(printf '%s' "$r" | jq '.results | length' 2>/dev/null)
    ...
    api_put "/api/config" "$ORIGINAL_CONFIG" >/dev/null; sleep 2
    log "Config restored"
}
...
test_provider_errors() {
    ...
    for _ in $(seq 1 5); do
        local r c
        r=$(api_get "/api/search?title=Flaky&year=2024&lang=en&type=movie"); sync_status
    ...
    local to timed_out
    to=$(api_get "/api/providers/timeout"); sync_status
    ...
    api_post "/api/providers/timeout/reset"; assert_status 200 "Timeout reset"
...
logging:
  ...
  format: json"
    ...
    api_put "/api/config" "$ORIGINAL_CONFIG" >/dev/null; sleep 1
}
...
test_language_rules() {
  ...
  rules:
    - audio: en
      subtitles:
        - code: fr'
    t=$(api_get "/api/search/targets?orig_lang=en&audio_langs=en"); sync_status
  ...
  rules:
    - audio: en
      subtitles:
        ...
        - code: de
        - code: es'
    api_get "/api/search/targets?orig_lang=en&audio_langs=en"; assert_status 200 "Lang: en -> fr,de,es"
  ...
  rules:
    ...
    api_get "/api/search/targets?orig_lang=fr&audio_langs=fr"; assert_status 200 "Lang: fr -> en std+forced"
  ...
  rules:
    - audio: en
      subtitles:
        - code: en
          variant: hi'
    api_get "/api/search/targets?orig_lang=en&audio_langs=en"; assert_status 200 "Lang: en -> en HI"
  ...
  rules:
    - audio: ja
      subtitles:
        - code: en
          providers: [mock]'
    api_get "/api/search/targets?orig_lang=ja&audio_langs=ja"; assert_status 200 "Lang: provider filter"
  ...
  rules:
    - audio: en
      subtitles:
        - code: fr
          min_score: 80'
    api_get "/api/search/targets?orig_lang=en&audio_langs=en"; assert_status 200 "Lang: min_score=80"
...
test_scans() {
    ...
    if [ -n "$sid" ] && [ "$sid" != "null" ]; then
        ...
        api_get "/api/activity"; assert_status 200 "Activity after scan"
    ...
    if [ -n "$mid" ] && [ "$mid" != "null" ]; then
        api_post "/api/scan/movie/${mid}"; assert_status 200 "Scan movie/$mid"
    ...
    # Edge cases
    api_post "/api/scan/series/0"; log "Scan series/0: HTTP $HTTP_STATUS"
    api_post "/api/scan/movie/0"; log "Scan movie/0: HTTP $HTTP_STATUS"
    api_post "/api/scan/item" "{}"; log "Scan item empty: HTTP $HTTP_STATUS"
    api_post "/api/scan/item" '{"media_type":"invalid","media_id":1}'; log "Scan item invalid: HTTP $HTTP_STATUS"
...
test_sync() {
    ...
    state=$(api_get "/api/state?limit=1"); sync_status
    ...
    api_post "/api/sync/offset" "{}"; log "Sync offset empty: HTTP $HTTP_STATUS"
}
...
test_poster_proxy() {
    ...
    mid=$(curl -sf --max-time 30 "${SUBFLUX_URL}/api/media/movies" 2>/dev/null | jq '.[0].id // empty')
    if [ -n "$mid" ] && [ "$mid" != "null" ]; then
        api_get "/api/preview/poster?type=movie&id=${mid}"; assert_status 200 "Poster: movie"
        api_get "/api/preview/poster?type=movie&id=${mid}&style=fanart"; log "Poster fanart: HTTP $HTTP_STATUS"
    else
    ...
    sid=$(curl -sf --max-time 30 "${SUBFLUX_URL}/api/media/series" 2>/dev/null | jq '.[0].id // empty')
    if [ -n "$sid" ] && [ "$sid" != "null" ]; then
        api_get "/api/preview/poster?type=series&id=${sid}"; assert_status 200 "Poster: series"
    else
        skip "No series for poster"
    fi

    api_get "/api/preview/poster?type=movie"; log "Poster no id: HTTP $HTTP_STATUS"
    api_get "/api/preview/poster?type=invalid&id=1"; log "Poster invalid type: HTTP $HTTP_STATUS"
}
...
test_hot_reload() {
    ...
    api_put "/api/config" "$modified"; assert_status 200 "Hot reload: debug logging"
...
test_real_providers() {
    ...
    r=$(api_get "/api/search?title=Inception&year=2010&lang=en&type=movie&imdb=tt1375666&tmdb=27205"); sync_status
    ...
    r=$(api_get "/api/search?title=Inception&year=2010&lang=fr&type=movie&imdb=tt1375666"); sync_status
    assert_status 200 "Real: French movie"
    total=$(printf '%s' "$r" | jq '.results | length' 2>/dev/null)
    log "Real: French returned $total results"

    r=$(api_get "/api/search?title=Bleach&season=1&episode=1&lang=en&type=episode&tvdb=74796"); sync_status
...
api_get "/api/config"; sync_status
